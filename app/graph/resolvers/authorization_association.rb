...
class Graph::Resolvers::AuthorizationAssociation < Graph::Resolvers::Base
  def self.build(preload, type: nil, &block)
    resolver_class = Class.new(self)
    resolver_class.define_method(:preload) { normalize_preloads(preload) }
    resolver_class.define_method(:type) { type }
    resolver_class.define_method(:handler) { block.present? ? block.call(object) : DefaultHandler }
