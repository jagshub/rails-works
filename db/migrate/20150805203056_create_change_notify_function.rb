class CreateChangeNotifyFunction < ActiveRecord::Migration
  def up
    ...
    execute <<-SQL
      CREATE FUNCTION change_trigger_v1() RETURNS trigger
      LANGUAGE plpgsql
      AS $$
        DECLARE
        BEGIN
    ...
    # execute <<-SQL
    #   CREATE OR REPLACE FUNCTION change_trigger_v1() RETURNS trigger AS
    #   $$
    #       var event = {};
    #
    ...
    #       event.version = 1;
    #
    #       plv8.execute("NOTIFY changes, '" + JSON.stringify(event) + "'");
    #   $$
    #   LANGUAGE "plv8";
    # SQL
  end
